    ...
    database.collection("Objects").findOne({objectId: req.params.objectId}, function (err, object) {
        if (err) {
            ...
            database.collection("Services").findOne({name: "log"}, function (err1, service) {
                ...
                // sql_db.query("SELECT * FROM `" + req.params.objectId + "` ORDER BY timestamp DESC LIMIT 0, " + (limit * limit), function (error, result) {
                sql_db.query("SELECT * FROM `" + req.params.objectId + "` ORDER BY timestamp", function (error, result) {
                    if (error) {
                        console.log("Error while getting logs from table " + req.params.objectId + ": ", error);
                    } else {
                        // var obj = {};
...
app.post("/apio/log", function (req, res) {
    ...
    if (req.body.type === "all") {
        query = "SELECT * FROM `" + req.body.objectId + "`" + whereClause + " ORDER BY `timestamp`";
        final();
        ...
        var follow = function () {
            ...
            database.collection("Objects").findOne({objectId: req.body.objectId}, function (err, object) {
                if (err) {
                    res.status(500).send(err);
                } else if (object) {
                    for (var i = 0; i < tsArray.length - 1; i++) {
                        ...
                        temp += ", " + tsArray[i] + " AS timestamp FROM `" + req.body.objectId + "` WHERE (timestamp >= " + tsArray[i] + " AND timestamp < " + tsArray[i + 1] + ")";
        ...
        if (endTimestamp === undefined && startTimestamp === undefined) {
            sql_db.query("SELECT MAX(`timestamp`) as `max`, MIN(`timestamp`) as `min` FROM `" + req.body.objectId + "`", function (error, result) {
                if (error) {
